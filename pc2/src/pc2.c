...
#define ATTACH_POINT "pc2"
const char* ic2 = "/net/i2/dev/name/local/ic2";
...
void *th_cc_passing (void *data){
  ...
  while(running){
    ...
    if(connected && pcs.passCCFlag == 1){
      ...
      msg.type = MESSAGE_PEDESTRIAN_LIGHT_CHANGE_PC2;
...
//Thread for msg passing to IC2

void *th_ic2_passing (void *data){
    ...
    unsigned connected = 0;
      ...
      if(!connected){
        if((server_coid = name_open(ic2, 0)) == -1){
          sleep(1);
          puts("failed to open global name for IC2");
        }else{
          connected = 1;
        }
      }
...
int main(void){
	...
	pthread_create (&th2, NULL, th_ic2_passing, NULL);
